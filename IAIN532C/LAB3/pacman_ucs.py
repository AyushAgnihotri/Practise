...
class node:
	def __init__(self, d, matrix, parent, x, y, xnew, ynew):
		self.matrix = matrix
		self.parent = parent
		self.x = xnew
		self.y = ynew
...
def solve(initial, p1, p2, f1, f2, r, c):
	...
	root = node(0, initial, None, p1, p2, p1, p2)
	...
	while (priority_queue):
		priority = sorted(priority_queue, key=lambda node: (node.d))
		...
		for i in range(4):
			...
			if (isSafe(parent.matrix, r, c, newx, newy)):
				...
				child = node(parent.d+1, newmatrix, parent, parent.x, parent.y, newx, newy)
