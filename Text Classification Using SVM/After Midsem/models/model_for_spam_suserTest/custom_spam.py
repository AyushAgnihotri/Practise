...
dataset = pd.read_csv('SMSSpamCollection.tsv', delimiter = '\t', quoting = 3)
...
while(1) :
    ...
    dataset = pd.concat([dataset,pd.DataFrame({'Article':[review],'Category' : ['ham'] },index = [originalArticles + countReview])])
    countReview += 1
...
for i in range(len(myreviews)) :
    ...
    if(choice == 'ham' or choice == 'Y' or choice == 'yes' or choice == 'YES' or choice == 'Yes' ) :
        fill = str(y_pred[i])
    else :
        fill = str(1 - y_pred[i])
        
    with open('SMSSpamCollection.tsv','a') as myfile :
